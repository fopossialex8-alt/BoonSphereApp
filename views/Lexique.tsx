    ...
    <div className="max-w-7xl mx-auto space-y-12 pb-24 animate-fade-in px-4">
      <header className="text-center py-20 bg-white rounded-[5rem] shadow-xl border-t-[10px] border-[#b08968]">
        ...
        <p className="text-[#b08968] font-black uppercase text-[10px] tracking-[0.5em] mt-4">
          <span className="font-sans normal-case">BɔɔnSphere</span> : {isLexique ? 'Présentation du lexique français-ngiembɔɔn' : 'Présentation du glossaire bilingue français-ngiembɔɔn'}
        </p>
                    ...
                    <tr key={idx} className="hover:bg-stone-50 transition-colors group">
                      ...
                      <td className="p-6 text-center">
                        <button 
                          ...
                          className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all mx-auto shadow-lg border-2 ${
                            playingId === idx 
                              ? 'bg-amber-500 text-white border-white animate-pulse' 
                              : 'bg-[#1a0f08] text-amber-500 border-amber-500/20 hover:bg-amber-500 hover:text-white'
                          }`}
                        >
                          <i className={`fas ${playingId === idx ? 'fa-spinner fa-spin' : 'fa-volume-high'}`}></i>
                        </button>
                      </td>
                    </tr>
                  ))
                    ...
                    <tr key={item.id} className="hover:bg-stone-50 transition-colors group">
                      ...
                      <td className="p-6 text-center">
                        <button 
                          ...
                          className={`w-14 h-14 rounded-xl flex items-center justify-center transition-all mx-auto shadow-xl border-2 ${
                            playingId === item.id 
                              ? 'bg-amber-500 text-white border-white animate-pulse' 
                              : 'bg-[#1a0f08] text-amber-500 border-amber-500/20 hover:bg-amber-500 hover:text-white'
                          }`}
                        >
                          <i className={`fas ${playingId === item.id ? 'fa-spinner fa-spin' : 'fa-volume-high'} text-xl`}></i>
                        </button>
                      </td>
                    </tr>
                  ))
