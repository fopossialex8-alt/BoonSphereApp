...
const TermCard: React.FC<TermCardProps> = ({ term, index }) => {
  ...
  const fallbackUrl = "https://images.unsplash.com/photo-1544333323-c242144ebd53?q=80&w=800&auto=format&fit=crop";
  const currentImageUrl = imgError ? fallbackUrl : (term.imageUrl || fallbackUrl);
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute top-4 left-4 bg-amber-600 text-white px-4 py-1 rounded-full text-[10px] font-black uppercase tracking-widest shadow-xl border-2 border-white/20 z-20">
          Fiche {index + 1}
        </div>

        <div className="absolute top-4 right-4 bg-white/10 backdrop-blur-md px-4 py-1.5 rounded-full text-[10px] font-black text-white shadow-lg border border-white/20">
          TERMINOLOGIQUE
        </div>
        ...
        <div className="absolute bottom-4 left-4 right-4 flex justify-between items-end">
          ...
          <button 
            onClick={handlePlay}
            disabled={isPlaying}
            className={`w-14 h-14 rounded-2xl flex items-center justify-center transition-all shadow-2xl border-2 ${
              isPlaying 
                ? 'bg-amber-500 text-white border-white animate-pulse scale-110' 
                : 'bg-white text-[#2d1b0d] border-amber-500 hover:bg-amber-500 hover:text-white'
            }`}
          >
            <i className={`fas ${isPlaying ? 'fa-spinner fa-spin text-xl' : 'fa-volume-high text-lg'}`}></i>
          </button>
        </div>
      </div>
